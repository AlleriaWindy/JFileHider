...
namespace Trinet.Core.IO.Ntfs
{
	...
	internal static class SafeNativeMethods
	{
        ...
        public static bool FileExists(string name) { return -1 != SafeGetFileAttributes(name); }
					...
					try
					{
						while (!finished)
						{
							...
							else
							{
								...
								{
                                    int a, b;
									if (!finished && !BackupSeek(hFile, streamId.Size.Low, streamId.Size.High, out a, out b, ref context))
									{
										finished = true;
									}
								}
